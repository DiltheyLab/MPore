...
- **Multipanel_plot_{file_name}.png**  
  ...
  If the plots are batched, please consider all batches together as one complete picture to interpret the underlying methylome results
Due to multiple enzymatic hits and visualization limitations, some complex strains were displayed as multiple multi-panel plots
...
The bar plot summarizes methyltransferase detection results for the benchmark dataset.
In addition, a representative multipanel plot generated by MPore for Mycoplasma hominis is shown.
For a detailed description of the methodology, analysis workflow, and reference Links, please refer to the MPore application note (https://doi.org/10.1093/bioadv/vbag077) and the supplementary data.
