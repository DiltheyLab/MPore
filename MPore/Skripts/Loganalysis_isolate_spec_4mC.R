...
main <- function(input_dir, output_dir, enzyme_df_path, dataframe_path, MTase_presence) {
 ...
 # MTase_df <- MTase_df[ ,c(TRUE, grepl("^M[0-9]*\\..+", colnames(MTase_df)[-1]))]  
  ...
  if (length(json_files) == 0) {
    json_files <- file.path(input_dir, "updated_25_4mC.json")
  }
  
  for (json_file in json_files) {
    # Extract the file number for output naming
    if (basename(json_file) == "updated_25_4mC.json") {
      file_number <- "all"
    } else {
   ...
   # updated_dict <- lapply(updated_dict, function(df) {
    ...
    for (key in names(updated_dict)) {
      ...
      if (nrow(df) == 0) {
        message(paste("⚠️  Dataframe", key, "is EMPTY after filtering"))
      } else {
        message(paste("✅ Dataframe", key, "has", nrow(df), "rows after filtering"))
      }

      print(paste("Dataframe:", key, " | mod_status class:", class(df$mod_status)))
      updated_dict[[key]] <- df
    }    
    ...
    updated_dict <- lapply(updated_dict, function(df) {
      keep_cols <- names(df) %in% c("mod_status", "check_status") | colSums(df == 0) != nrow(df)
      df[, keep_cols, drop = FALSE]
    })
   ...
   #beta_vec <- beta_vec[beta_vec > 0.5]
    ...
    #write.xlsx(Excel_df, file.path(output_dir, paste0("Context_influence_4mC_", file_number, ".xlsx")))
    #print("Context 4mC influence DOne")
    write.csv(expanded_result_df_filt, file.path(output_dir, paste0("Beta_coef_p_values_filt_4mC_2", file_number, ".csv")), row.names = FALSE)
    write.xlsx(expanded_result_df_filt, file.path(output_dir, paste0("Beta_coef_p_values_filt_4mC_2", file_number, ".xlsx")))
