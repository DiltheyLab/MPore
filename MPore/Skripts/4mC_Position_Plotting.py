...
def create_boxplots_with_data(dataframes_list, x_values, contig_positions_dict, enzyme_count_df, log_results_df, output_dir):
    ...
    for motif in unique_motifs:
        ...
        ax_box.set_title(f'4mC Boxplot for {motif}', fontsize=16)
        ...
        #plt.show()
        # Save figure
        filename = os.path.join(output_dir, f'{motif}_4mC_boxplot.png')
...
def main():
    ...
    # Load 5mC Files
    matched_files_5mC = glob.glob(os.path.join(directory, '*_detailed_*_4mC.csv'))
    dataframes_list = []
    for file_path in matched_files_5mC:
        base_name = os.path.basename(file_path)
        if any(f"Sample_DF_detailed_{file_name}_4mC.csv" == base_name for file_name in file_names):
            try:
    ...
    matched_beta_csv=[]
    matched_beta_csv= glob.glob(os.path.join(directory, 'Beta_coef_p_values_filt_4mC_*.csv'))
    ...
    file_name
    for file_path in matched_files_5mC
        if (file_name := os.path.basename(file_path).split('_detailed_')[1].split('_4mC')[0]) in file_names
    ]
