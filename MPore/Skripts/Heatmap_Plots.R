...
#Funktion um einen marker zum Motif hinzuzufügen 
...
#Output_2 <- "/Users/azlannisar/Desktop/mount2/Myco_Data/Output"

#Context <- "MetaMyco_new_db"
#Look for pattern in the subdict
...
#We only need 1 cause it contains the information of all contexts anyways 
filtered_paths <- file_paths[grepl("plots_6mA/enzyme_presence_df\\.csv$", file_paths)]
gene_df <- read.csv(Gene_df)
#Set up some empty dataframes 
all_dataframes <- list()
Enzyme_combined_data <- data.frame()
combined_df <- data.frame()
combined_VRE <- data.frame()
#Get Enzyme_combined_data it includes the presence and e-value of the enzymes detected in each isolate 
...
#subset the entries and combine them to generate an enzyme and e-vlaue overview
subset_list <- list()
...
#check nach !na und für True 1 und false bleibt 0
...
if ("5mC" %in% Methyltypes){
  ...
  for (isolate in load_isolate) {
    ...
    #subdirs <- list.dirs(path = Path_to_Score_Data, full.names = TRUE, recursive = FALSE)
    #subdirs_output <- subdirs[grepl(Output_2, subdirs, fixed = TRUE)]
    #files <- unlist(lapply(subdirs_output, function(subdir) {
      #list.files(path = subdir, pattern = pattern, full.names = TRUE)
    #}))
    #file_names[[isolate]] <- files
  }
  #An empty dataframe structure
  result_df <- data.frame(Key = character(0))
  #We load each Sample_DF_isolate_5mC.csv into R and load an result_df for the heatmap
  ...
  #Filter Motifs out wir no "C" since they dont make any sense in a 5mC context
  result_df <- result_df[grep("C", result_df$Key), ]
  result_df$Key <- gsub("\\s*\\(\\+\\)$", "", result_df$Key)
  result_df$Key <- gsub("\\s*\\(\\-\\)$", "", result_df$Key)
  
  #Filter only motifs hit by the enzyme from genimic identification
  ...
  #mapply durch jede zeile von 5mC tabelle und markiere jenach position das motif 
  ...
  #distance_matrix <- dist(heatmap_matrix_5mC)  
  ...
  #Markierungsansatz für die Enzyme die laut homologie search vorhanden sind 
  ...
  #heatmap_matrix_filtered <- heatmap_matrix_5mC[rownames(heatmap_matrix_5mC) %in% rownames(final_marking_matrix), ]
...
if ("6mA" %in% Methyltypes) {
  ...
  for (isolate in load_isolate) {
    ...
    #subdirs <- list.dirs(path = Path_to_Score_Data, full.names = TRUE, recursive = FALSE)
    #subdirs_output <- subdirs[grepl("Output", subdirs)]
    #files <- unlist(lapply(subdirs_output, function(subdir) {
      #list.files(path = subdir, pattern = pattern, full.names = TRUE)
    #}))
    #file_names_6mA[[isolate]] <- files
  ...
  #Filter Motifs out wir no "A" since they dont make any sense in a 6mA context
...
##This part checks which mod_files are there. and combined them into one file 
...
# Add common elements for all layers (if applicable)
...
#Rekursive search for beta coef files 
...
#Create the Matrix in which enzyme is column and motifs are columns to set up a bindary meatrix for enzyme hit per motif
...
#Adjust the matrix to the changes 
...
#Only if we keep the beta< 0.1 is 0 to reduce the noise 
...
#Output_2 <- "/Users/azlannisar/Documents/"
#Panel A 
#New Panel A first we get the enzyme per Isolate overview
...
#With this part we got the Dataframe for the new Panel A it include the beta values the enzymes and isolates relationship and the empirical scores 
panel_A_ggplot <- ggplot(final_df, aes(x = Isolate, y = Grouped_name, fill = E_value)) +
 ...
 # geom_tile(data = subset(final_df, Border == "1"),
           # aes(x = Isolate, y = Enzyme), color = "black", linewidth = 1, fill = NA) +   
 # geom_text(aes(label = sprintf("%.0f", Presence)), color = "black", size = 12) +
  scale_fill_gradient(
    low = "lightblue",  
    high = "blue", 
    na.value = NA,
...
for (isolate in isolates) {
  ...
  # Filter the dataframes based on the current isolate
  ...
  # Modify Motif factor levels to ensure consistent order
  ...
  # Create the barplot for beta coefficient
  ...
  # Create the barplot for methylation score
  ...
  # Save the plot for the current isolate
