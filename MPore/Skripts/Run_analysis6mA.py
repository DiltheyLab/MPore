...
def generate_motif_dataframes(matches, strand):
    ...
    for motif, positions in matches.items():
        ...
        for pos, contig_number in positions:  # Unpack position and contig number
            ...
            if motif.count('A') > 1:
                next_c_index = motif.find('A', i+1)  # Find next 'C' after the current index i
                while next_c_index != -1:
                    ...
                    next_c_index = motif.find('A', next_c_index+1)  # Find next 'C'
...
def main():
    ...
    #data_df = data_df[data_df['File_name'].isin(file_names)]
