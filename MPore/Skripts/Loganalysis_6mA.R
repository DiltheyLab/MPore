 ...
 # MTase_df <- MTase_df[ ,c(TRUE, grepl("^M[0-9]*\\..+", colnames(MTase_df)[-1]))]  
  ...
  for (json_file in json_files) {
   ...
   #beta_vec <- beta_vec[beta_vec > 0.5]
    if (length(beta_vec) > 0) {
      ...
      if (nrow(Enzymatic_entries) == 0) {
        ...
        for (i in 1:nrow(Enzymatic_entries)) {
          ...
          print(current_row)
          enzyme_name <- current_row$Enzyme
          if (grepl("_", current_row$Enzyme) && !(enzyme_name %in% names(updated_dict))) {
    ...
    write.xlsx(Excel_df, file.path(output_dir, paste0("Context_influence_6mA_", file_number, ".xlsx")))
    ...
    write.csv(lambda_df, file.path(output_dir, paste0("Lambda_values_", file_number, ".csv")), row.names = FALSE)
  }
}
