 ...
 # MTase_df <- MTase_df[ ,c(TRUE, grepl("^M[0-9]*\\..+", colnames(MTase_df)[-1]))]  
  ...
  for (json_file in json_files) {
   ...
   #beta_vec <- beta_vec[beta_vec > 0.5]
    ...
    #write.xlsx(Excel_df, file.path(paste0(output_dir, paste0("Context_influence_6mA_", file_number, ".xlsx"))))
    #print("Context 6mA influence DOne")
    write.csv(expanded_result_df_filt, file.path(output_dir, paste0("Beta_coef_p_values_filt_6mA_2", file_number, ".csv")), row.names = FALSE)
    write.xlsx(expanded_result_df_filt, file.path(output_dir, paste0("Beta_coef_p_values_filt_6mA_2", file_number, ".xlsx")))
