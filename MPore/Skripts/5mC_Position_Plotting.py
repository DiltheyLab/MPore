...
def iupac_can_be(base: str, target: str) -> bool:
    iupac_to_regex_2 = {
        'A': '[AWRMDHVN]',
        'T': '[TYWKBDHVN]',
        'C': '[CYSBDHVN]',
    ...
    return target in iupac_to_regex_2.get(base.upper(), '')
def count_possible_bases(motif, base):
    return sum(iupac_can_be(nuc, base) for nuc in motif)
...
def process_data(df):
    ...
    for _, row in df.iterrows():
        ...
        if len(positions) == 1 and positions[0] != '?':
            ...
            for i, nucleotide in enumerate(motif_copy):
                ...
                if unknown_methyl_type in ['5mC', '4mC']:
                    if count_possible_bases(motif, 'C') > 1 and iupac_can_be(nucleotide, 'C'):
                        enzyme_unique_name = f"{enzyme}_C{i+1}"

                elif unknown_methyl_type == '6mA':
                    if count_possible_bases(motif, 'A') > 1 and iupac_can_be(nucleotide, 'A'):
                ...
                if enzyme_unique_name:
                    ...
                    if known_position.isdigit():
                        result_rows.append([row['Enzyme'], row['Enzyme'], row['Motif'], known_methyl_type, known_strand, known_position])
    ...
    result_df = result_df.drop_duplicates(subset=['Enzyme', 'EnzymeUniqueMethylationActivity', 'Motif', 'MethylationType', 'Strand', 'Position'])
    return result_df
...
def count_enzyme_positions_for_motifs_fixed(contig_positions_dict, dataframes_list, x_values):
    ...
    for motif in unique_motifs:
        ...
        for isolate in x_values:
            # Retrieve the contig positions and the corresponding dataframe for the isolate
            sp_dict = contig_positions_dict[isolate]
            sp_index = x_values.index(isolate)
            df_sp = dataframes_list[sp_index]

            # Filter the dataframe for rows matching the current motif
            motif_data = df_sp[df_sp['Motif'] == motif].copy()
            motif_data['Contig'] = motif_data['Contig'].astype(str)

            
            # Get unique (Mod_Pos, Contig) combinations
            unique_mod_pos = motif_data[['Mod_Pos', 'Contig']].drop_duplicates()

            # Iterate over each (Mod_Pos, Contig) pair
            for mod_pos, contig in unique_mod_pos.values:
                # Check if the (Mod_Pos, Contig) exists in contig_positions_dict
                if (str(contig), mod_pos) in sp_dict:
                    entries = sp_dict[(str(contig), mod_pos)]
                    
                    # Iterate over the entries for the matched (Contig, Mod_Pos)
                    for entry in entries:
                        # If the methylation type is '6mA'
                        if entry['MethylationType'] == '5mC':
                            enzyme = entry['Enzyme']
                            # Increment the enzyme-motif count for the current isolate
                            enzyme_count_dict[isolate][enzyme][motif] += 1
                            
    # Collect the results into a list for DataFrame creation
    data = []
...
def create_boxplots_with_data(dataframes_list, x_values, contig_positions_dict, enzyme_count_df, log_results_df, output_dir):
    ...
    # Filtere Motive, die kein 'A' enthalten, direkt in der äußeren Schleife
    for motif in unique_motifs:
        if '"C"' not in motif:
            continue
        ...
        fig_width = max(12, len(x_values) * 1.6)
        ...
        ax_box.set_title(f'5mC Boxplot for {motif}', fontsize=16)
        ...
        #plt.show()
        # Save figure
        filename = os.path.join(output_dir, f'{motif}_5mC_boxplot.png')
...
def main():
    ...
    #csv_file_path= "/Users/azlannisar/Desktop/mount/Data_Test2.csv"
    ...
    #file_names = {"barcode02", "barcode11"}
    ...
    for file_path in matched_files_5mC:
        ...
        if any(f"Sample_DF_detailed_{file_name}_5mC.csv" == base_name for file_name in file_names):
            try:
    ...
    #Loading the Beta_coef_files and filtering accodingly 
    matched_beta_csv=[]
    matched_beta_csv= glob.glob(os.path.join(directory, 'Beta_coef_p_values_filt_5mC_*.csv'))
    ...
    #Now filter Isolate column == one of the filenames all other remove 
    Log_Results_df = Log_Results_df[Log_Results_df['Isolate'].isin(file_names)]
        
    # Read TSV file
   #tsv_file_path="./TSV_REBASE_data.tsv"
    tsv_file_path = "/home/azlan/VRE_Data/TSV_Enzyme.csv"
    ...
    file_name
    for file_path in matched_files_5mC
        if (file_name := os.path.basename(file_path).split('_detailed_')[1].split('_5mC')[0]) in file_names
    ]
    ...
    #This part has to come back its better to refine the reference names before running this instead of the other way around
    #also changing the string values to let the script run on mac has to change back for vader or hpc 
