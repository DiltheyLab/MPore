 ...
 # MTase_df <- MTase_df[ ,c(TRUE, grepl("^M[0-9]*\\..+", colnames(MTase_df)[-1]))]  
  ...
  for (json_file in json_files) {
   ...
   # updated_dict <- lapply(updated_dict, function(df) {
    ...
    write.csv(expanded_result_df_filt, file.path(output_dir, paste0("Beta_coef_p_values_filt_4mC_", file_number, ".csv")), row.names = FALSE)
    write.xlsx(expanded_result_df_filt, file.path(output_dir, paste0("Beta_coef_p_values_filt_4mC_", file_number, ".xlsx")))
