...
# Create List for different Isolates 
for (file in files) {
...
# Create a Dataframe where columns are enzymes and isolates are rows, shows how many enzymes are per evalue threshold
...
#print(identifier_df)
#write.table(identifier_df, "/Users/azlannisar/Blast_Result_DF_analysis.csv", sep = "\t", row.names = FALSE)
#write.xlsx(identifier_df, file = "/Users/azlannisar/Blast_Result_analysis.xlsx")

# Check the Entries of each Threshold
...
#enzyme_presence_df_2 <- enzyme_presence_df_2 %>%
  #select(Isolates, matches("^M[0-9]*\\."))
