...
def main():
    ...
    #Sanity check 1 for motif hits in specific dataframes_lists 
    #first_df = dataframes_list[1]
    #count_minus_df = dataframes_list[1][
    #(dataframes_list[1]['Motif'] == 'GC"A"TC') & (dataframes_list[1]['Strand'] == '-')
#]
    #count_plus_df = dataframes_list[1][
    #(dataframes_list[1]['Motif'] == 'GC"A"TC') & (dataframes_list[1]['Strand'] == '+')
#]
    

    #motif_filter = first_df['Motif'].str.contains(r'GC"A"TC', regex=True)
    #count = first_df['Motif'].str.contains(r'GC"A"TC', regex=True).sum()
    #print(f"Number of entries with the motif 'GC\"A\"TC': {count}")
    #count_plus = first_df[motif_filter & (first_df['Strand'] == '+')].shape[0]
    #count_minus = first_df[motif_filter & (first_df['Strand'] == '-')].shape[0]
    #print(f"Entries with Strand '+': {count_plus}")
    #print(f"Entries with Strand '-': {count_minus}")
