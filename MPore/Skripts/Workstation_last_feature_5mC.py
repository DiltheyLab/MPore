...
def main():
   ...
   # matched_files_5mC = glob.glob(os.path.join(directory, '*_detailed_*_6mA.csv'))
    matched_files_5mC = [
        file for file in glob.glob(os.path.join(directory, '*_detailed_*_5mC.csv'))
        if any(f"Sample_DF_detailed_{file_name}_5mC.csv" in file for file_name in file_names)]
    dataframes_list = []
    ...
    #x_values = [os.path.basename(file_path).split('_detailed_')[1].split('_5mC')[0] for file_path in matched_files_5mC]
    ...
    for key, updated_df in updated_dict.items():
        ...
        for contig_number, enzyme_info_list in combined_contig_data.items():
            for enzyme_info in enzyme_info_list:
                ...
                if enzyme_name and methylation_type == '5mC' and pos is not None:
    ...
    # Calculate the number of splits (each dictionary with 3 DataFrames)
     ...
     for i in range(num_splits):
         ...
         split_filename = os.path.join(output_dir, f"updated_25_5mC_split_{i + 1}.json")
        ...
        json_filename = os.path.join(output_dir, 'updated_25_5mC.json')
        with open(json_filename, 'w') as f:
            json.dump(serializable_dict, f)
        print("Saved updated_25_5mC.json without splitting.")
